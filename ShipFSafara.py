B#mple for accessing smeared hits and fitted tracks
...
def myEventLoop(n):
  ...
  if l > 1:
    ...
    for HNL in sTree.Particles:
      ...
      t=t1
      while t<=t2: 
        ID.append(str(sTree.FitTracks[t].getFittedState().getPDG()))
        t+=1
      strID = " ".join(ID)      
      if strID not in c:
        c[strID]=0
      ...
      #if Daughter1.getPDG() not in c:
      #  c[Daughter1.getPDG()]=0

      #if Daughter2.getPDG() not in c:
      #  c[Daughter2.getPDG()]=0

      #c[Daughter1.getPDG()]+=1
      # c[Daughter1.getPDG()]+=1
	...
	# Need to remember to change their values when analyzing DPs and SGs
        data_file.write(str(tot_P)+", ")
        data_file.write(str(tot_Pt)+", ")
        data_file.write(str(tot_Pt/tot_P)+", ")
        data_file.write(str(Opening_angle)+", ")  
        data_file.write(str(Decay_angle)+", ")  
        data_file.write(str(Impact_par)+", ") 
        data_file.write(str(mom1.Mag())+", ")  
        data_file.write(str(p1.Pt())+", ")  
        data_file.write(str(p1.Pt()/mom1.Mag())+", ")  
        data_file.write(str(mom2.Mag())+", ")  
        data_file.write(str(p2.Pt())+", ")  
        data_file.write(str(p2.Pt()/mom2.Mag())+", ")  
        data_file.write(str(HNLPos.Z())+", ")  
        data_file.write(str(HNLPos.X())+", ")  
        data_file.write(str(HNLPos.Y())+", ")  
        data_file.write(str(1))  
        data_file.write("\n") 
...
data_file_name="kinematic_variables.dat"
...
data_file.write("Mother Total Momentum (GeV/c), ")
data_file.write("Mother Transverse Momentum (GeV/c), ")
data_file.write("Mother Fraction of Transverse Momentum, ")
data_file.write("Opening Angle (rad), ")  
data_file.write("Decay Angle (rad), ")  
data_file.write("Impact Parameter (cm), ") 
data_file.write("Daughter1 Total Momentum (GeV/c), ")  
data_file.write("Daughter1 Transverse Momentum (GeV/c), ")  
data_file.write("Daughter1 Fraction of Transverse Momentum, ")
data_file.write("Daughter2 Total Momentum (GeV/c), ")  
data_file.write("Daughter2 Transverse Momentum (GeV/c), ")  
data_file.write("Daughter2 Fraction of Transverse Momentum, ")
data_file.write("Decay Z (cm), ")
data_file.write("Decay X (cm), ")
data_file.write("Decay Y (cm), ")
data_file.write("Event type")  
data_file.write("\n")
